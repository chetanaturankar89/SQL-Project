...
-- count of movie table 
SELECT 
    COUNT(*) AS count_of_movie
FROM
    movie;

-- count of genre table 
SELECT 
    COUNT(*) AS count_of_genre
FROM
    genre;

-- count of director_mapping table 
SELECT 
    COUNT(*) AS count_of_director_mapping
FROM
    director_mapping;

-- count of role_mapping table 
SELECT 
    COUNT(*) AS count_of_role_mapping
FROM
    role_mapping;

-- count of names table 
SELECT 
    COUNT(*) count_of_names
FROM
    names;
...
-- Null duration count 
SELECT 
    COUNT(*) AS duration_nulls_values
FROM
    movie
...
-- Null country count 
SELECT 
    COUNT(*) AS country_null_values
FROM
    movie
WHERE country IS NULL;

-- Null worlwide_gross_income  count 
...
-- production_company null values
SELECT 
    COUNT(*) AS production_company_null_values
FROM
    movie
...
+---------------+-------------------+ */
-- Type your code below:
-- part one -->  Find the total number of movies released each year? 
/* Output format for the first part:
...
-- Type your code below:
SELECT 
    year, count(*) number_of_movies
FROM
    movie
group by year ;

-- part two -->  How does the trend look month wise? (Output expected)
...
SELECT 
    genre, COUNT(*) no_or_movies
FROM
    genre
GROUP BY genre
ORDER BY no_or_movies DESC
LIMIT 1;
...
FROM
    genre
GROUP BY movie_id) 
select count(distinct movie_id) as  only_one_genre
from  genre_summary where genre_count = 1;
...
ORDER BY genre , median_rating DESC;
...
WITH Higher_votes as (
SELECT
     COUNT(CASE WHEN (m.languages) like '%german%' THEN m.id END) AS german_movie,
     COUNT(CASE WHEN LOWER(m.languages) LIKE '%italian%' THEN m.id END) AS italian_movie,
	 SUM(CASE WHEN LOWER(m.languages) LIKE '%german%' THEN r.total_votes END) AS sum_german_movie_votes,
	 SUM(CASE WHEN LOWER(m.languages) LIKE '%italian%' THEN r.total_votes END) AS sum_italian_movie_votes
...
ON m.id = r.movie_id )
SELECT 
    ROUND(sum_german_movie_votes / german_movie, 2) AS german_votes_per_movie,
    ROUND(sum_italian_movie_votes / italian_movie, 2) AS italian_votes_per_movie
FROM
    Higher_votes;
...
-- Type your code below:
-- CTE
WITH top_three_production_houses AS
(
SELECT 
    m.production_company,
...
with moving_avg_summ as (
select g.genre, round(sum(m.duration)/count(m.duration),2) as avg_duration
from genre g left join movie m
on g.movie_id=m.id
group by g.genre
)
select * ,
SUM(avg_duration) OVER (order by mo.genre) as running_total_duration,
AVG(avg_duration) OVER (order by mo.genre) as moving_average_duration
from moving_avg_summ mo;
...
on m.id=r.movie_id
where POSITION(',' IN languages)>0 and median_rating>=8 and m.production_company is not null
group by m.production_company
...
With act_rank_summ as(
select n.name as actress_name,sum(r.total_votes) as total_votes, count(r.movie_id) as movie_count, sum(r.avg_rating*r.total_votes)/sum(r.total_votes) as actress_avg_rating, 
RANK() OVER (order by sum(r.avg_rating*r.total_votes)/sum(r.total_votes) desc) as actress_rank1
...
where g.genre='Drama' and ro.category='Actress' and r.avg_rating>8
group by n.name
order by sum(r.avg_rating*r.total_votes)/sum(r.total_votes) desc),
row_wise_rank_summ as(
select *,
ROW_NUMBER() Over(partition by actress_rank1 order by actress_name) as row_wise_rank
from act_rank_summ
order by actress_rank1),

final_rank_summ as(
select *,
ROW_NUMBER() Over(order by actress_rank1) as actress_rank
from row_wise_rank_summ
order by actress_rank1)

select actress_name,total_votes,movie_count,actress_avg_rating,actress_rank from final_rank_summ
